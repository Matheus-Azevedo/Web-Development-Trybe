...
console.table(youngling(employees));

// ----------------------------------------------------------------------

const older = (lista) => {
  return lista.filter((element) => element.Age >= 40);
}
console.table(older(employees))

const employeesWithCar = (lista) => {
  lista.map((element) => {
    if (element.Age > 35) {
      return element.Car = true;
    }
    return element.Car = false;
  });
  return lista;
} 

console.table(employeesWithCar(employees));
